#RDS Build Orchestrator for High Availability Build
...
#search and replace "$gatewayfqdn1" with fqdn of front end of rds gateway like "rd.domain.com" no quotes
#search and replace "$clientaccessname1" with fqdn of rds broker client accessname like "rds.domain.com" no quotes
#search and replace "$web1ip", $web2ip, $brkr1ip, $brkr2ip and fill in the respective IPs
...
$brkr1="broker1 server fqdn"
$brkr2="broker 2 server fqdn"
$clientaccessname = "rds.domain.com"
...
$sqlserver="name of sql server(select the one that has spn set up, could be netbios name only or fqdn, must look in ad"
$dbname="name of rds database"
...
Add-DnsServerResourceRecordA -Name "$gatewayfqdn1" -ZoneName "$dnsdn" -AllowUpdateAny -IPv4Address "$web1ip" -TimeToLive 00:00:00
Add-DnsServerResourceRecordA -Name "$gatewayfqdn1" -ZoneName "$dnsdn" -AllowUpdateAny -IPv4Address "$web2ip" -TimeToLive 00:00:00
Add-DnsServerResourceRecordA -Name "$clientaccessname1" -ZoneName "$dnsdn" -AllowUpdateAny -IPv4Address "$brkr1ip" -TimeToLive 00:00:00
Add-DnsServerResourceRecordA -Name "$clientaccessname1" -ZoneName "$dnsdn" -AllowUpdateAny -IPv4Address "$brkr2ip" -TimeToLive 00:00:00
...
Set-RDConnectionBrokerHighAvailability `
	-ConnectionBroker $brkr1 `
	-DatabaseConnectionString "DRIVER={ODBC Driver 17 for SQL Server};SERVER=$sqlserver;Trusted_Connection=yes;DATABASE=$dbname"
	-clientaccessname $clientaccessname1

Add-RDServer -Server "$brkr2" -Role "RDS-CONNECTION-BROKER" -ConnectionBroker "$brkr1"
...
$computers = "$web1","$web2","$brkr1","$brkr2","$rdsh1","$rdsh2","$nps"
