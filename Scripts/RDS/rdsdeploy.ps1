...
#search and replace "$groupsid1" with actual groupsid_output  from below command no quotes
...
##Copy below output of $groupsid variable in ConfigureRDSGateway.ps1
$groupsid
...
New-Item -ItemType "directory" -Path "RDS:\gatewayserver\NPSServers" -name $nps1
...
$name="RD_RAP"
new-rdsgwrap -name $name -usergroupnames $rdsusergroup
} -computername $web1
...
Read-host -prompt "Run configurenps.ps1 on NPS Server.  Press Enter when finished, A RDS Session will be created for you."
